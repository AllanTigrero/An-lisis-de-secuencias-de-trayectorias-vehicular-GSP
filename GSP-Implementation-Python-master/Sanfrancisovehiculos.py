...
class GSP:
        ...
        for filename in files:
          ...
          #  print (full_path)
            ...
            for index, row in df.iterrows():
                ...
                cant_vehiculos = row['cant_vehiculos']
                #grupo = row['grupo']
                #lat_minima = row['lat_minima']
                # Creando el diccionario x
                x = {'grupo_long': grupo_long, 'grupo_lat': grupo_lat, 'cant_vehiculos': cant_vehiculos}
                if x:
                    ans[index] = x
                    s1.append(x)
    ...
    def write_to_csv(self, output_file):
        with open(output_file, 'w', newline='') as csvfile:
            ...
            csvwriter.writerow(['grupo_long', 'grupo_lat', 'cant_vehiculos', 'support'])
            for pattern, support in self.items.items():
                values = {}
                values["grupo_long"] = pattern[0][1]
                values["grupo_lat"] = pattern[1][1]
                values["cant_vehiculos"] = pattern[2][1]
                csvwriter.writerow([values['grupo_long'], values['grupo_lat'], values['cant_vehiculos'], support])
    ...
    def get_support_items(self):
        ...
        print(" grupo_long", " grupo_lat"," cant_vehiculos"," support")
        for t in sorted_patterns:
            ...
            values["grupo_lat"] = pattern[1][1]
            values["cant_vehiculos"] = pattern[2][1]
            print(values['grupo_long'], values['grupo_lat'], values['cant_vehiculos'], support)
        
        with open("SANFRANCISCO3.csv", 'w',) as f:
            writer = csv.writer(f)
            writer.writerow(['grupo_long', 'grupo_lat', 'cant_vehiculos', 'support'])
            for t in sorted_patterns:
                pattern = t[0]
                support = t[1]
                values = {}
                ...
                values["grupo_lat"] = pattern[1][1]
                values["cant_vehiculos"] = pattern[2][1]
                writer.writerow([values['grupo_long'], values['grupo_lat'], values['cant_vehiculos'], support])    

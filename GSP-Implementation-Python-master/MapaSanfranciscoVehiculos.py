...
datos = pd.read_csv("SANFRANCISCO3.csv", sep=",",decimal=".")

rango = 0.0003
datos['lon_minima'] = -122.4066 + (datos['grupo_long']-1) * rango
datos['lon_maxima'] = -122.4066 + (datos['grupo_long']+0) * rango
datos['lat_minima'] = 37.7888 + (datos['grupo_lat']-1) * rango
datos['lat_maxima'] = 37.7888 + (datos['grupo_lat']+0) * rango
...
colors = ["white","lightblue","orange","red","green","blue","yellow","magenta","brown","purple"]
color_labels = ["Secuencias 0", "Secuencias 1", "Secuencias 2", "Secuencias 3", "Secuencias 4", "Secuencias 5", "Secuencias 6", "Secuencias 7", "Secuencias 8", "Secuencias 9", "Secuencias 10", "Secuencias 11", "Secuencias 12", "Secuencias 13", "Secuencias 14", "Secuencias 15"]
# Crear el mapa base (mapa interactivo)
mapa = folium.Map(location=[center_lat, center_lon], zoom_start=15)
legend_html = '''
     <div style="position: fixed; 
     background: rgba(255,255,255,0.8); bottom: 50px; left: 50px; width: 150px; height: 250px; 
...
for i in range(len(datos)):
    ...
    folium.Marker(
        ...
        +"<br> Cantidad de vehiculos: "+str(datos['cant_vehiculos'][i]),
        tooltip=tooltip
    ).add_to(mapa)

mapa.save("MapaSanFranciso3.html")
